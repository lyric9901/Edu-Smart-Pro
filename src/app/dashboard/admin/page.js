    ...
    <div className={`min-h-screen transition-colors duration-300 ${theme === 'dark' ? 'bg-black text-zinc-100' : 'bg-slate-50 text-slate-900'} font-sans p-4 md:p-8`}>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <motion.div 
            ...
            className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-[2rem] p-6 text-white shadow-xl shadow-blue-500/20 flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden"
        >
          ...
          <div className="flex-1 relative z-10">
              ...
              <p className="text-blue-100 text-sm mt-1 max-w-lg font-medium">
                  Students can use this link to connect instantly to <strong>{user?.username}'s Coaching</strong>. No codes needed.
              </p>
          </div>
          <div className="flex w-full md:w-auto bg-black/20 backdrop-blur-md rounded-2xl p-1.5 items-center border border-white/10 relative z-10">
              <code className="flex-1 px-4 py-2 text-xs md:text-sm font-mono text-white/90 truncate max-w-[200px] md:max-w-none">
                  {typeof window !== 'undefined' ? window.location.host : "..."}/?schoolId={user?.schoolId}
              </code>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="bg-white dark:bg-zinc-900 p-5 rounded-[2rem] shadow-sm border border-slate-200 dark:border-zinc-800 h-fit">
            ...
            <div className="space-y-2">
              {loading ? (
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between p-4 border border-slate-100 dark:border-zinc-800 rounded-2xl"><Skeleton className="h-4 w-20"/><Skeleton className="h-6 w-8 rounded-full"/></div>
                  </>
              ) : batches.length > 0 ? (
                  <>
            ...
            {selectedBatch ? (
              <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2rem] shadow-sm border border-slate-200 dark:border-zinc-800 min-h-[500px] flex flex-col">
                <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 pb-4 border-b border-slate-100 dark:border-zinc-800 gap-4">
                  <div>
                    <h2 className="text-2xl font-black text-slate-800 dark:text-white">{selectedBatch.name}</h2>
                  ...
                  <div className="flex gap-2">
                      <div className="relative">
                          ...
                          <input 
                            placeholder="Search..." 
                            className="pl-9 pr-4 py-2 bg-slate-50 dark:bg-black border border-slate-200 dark:border-zinc-800 rounded-xl text-sm outline-none focus:border-blue-500 transition-colors w-32 md:w-auto"
                            value={searchQuery}
                      ...
                      {filteredStudents.map((student, i) => (
                        ...
                        >
                          <td className="p-4 font-bold text-slate-800 dark:text-zinc-200 flex items-center gap-3">
                              <div className="w-9 h-9 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 flex items-center justify-center text-xs font-black shadow-sm">
                                  {student.name.charAt(0)}
                              </div>
                              {student.name}
                          </td>
                          <td className="p-4 text-slate-500 font-mono font-medium">{student.phone}</td>
                          <td className="p-4">
