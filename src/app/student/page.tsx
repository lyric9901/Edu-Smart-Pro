...
function StudentContent() {
    ...
    const navItems = [
        ...
        { id: 'settings', icon: Settings, label: 'Settings', desc: 'Customize preferences and account', color: 'text-purple-500', bg: 'bg-purple-50 dark:bg-purple-500/10' },
    ];
    ...
    return (
        <div className="app-shell flex min-h-screen font-sans transition-colors duration-300 select-none text-slate-900 dark:text-zinc-100 relative overflow-hidden bg-slate-50 dark:bg-[#030712]">
            ...
            {/* Desktop Sidebar (Cleaner layout with items moved to settings) */}
            <aside className="hidden md:flex flex-col w-72 fixed h-full glass-panel z-30 p-5 border-r border-slate-200/50 dark:border-white/5 bg-white/40 dark:bg-[#06080f]/80 backdrop-blur-2xl">
                <div className="p-4 mb-6">
                    <h1 className="text-xl font-black text-slate-950 dark:text-white tracking-tighter flex items-center gap-2">
                        {schoolName}
                    </h1>
                </div>
                    ...
                    {navItems.map(item => {
                        ...
                        return (
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-colors duration-200 font-bold text-sm border ${activeTab === item.id && item.id !== 'settings' ? 'bg-white shadow-sm border-slate-200 text-blue-600 dark:bg-white/10 dark:border-white/10 dark:text-white' : 'border-transparent text-slate-600 hover:bg-white/60 hover:text-slate-950 dark:text-zinc-400 dark:hover:bg-white/5 dark:hover:text-zinc-200'}`}
                            >
            ...
            <main className="flex-1 md:ml-72 relative z-10 flex flex-col min-h-screen overflow-x-hidden pt-4 md:pt-10">
                ...
                {/* Tab Content Wrapper */}
                <div className="px-4 md:px-10 pb-12 max-w-6xl mx-auto pt-2 md:pt-0 w-full flex-1">
                    
                    {/* Compact Mobile Back Header for Sub-tabs */}
                    {activeTab !== 'dashboard' && (
                        <div className="md:hidden flex items-center gap-3 mb-6">
                             <motion.button whileTap={{ scale: 0.95 }} onClick={() => setActiveTab('dashboard')} className="touch-target rounded-xl bg-white dark:bg-[#0b1120] border border-slate-200 dark:border-white/5 flex items-center justify-center shadow-sm">
                                 <ChevronLeft size={20} className="text-slate-700 dark:text-zinc-200" />
                             </motion.button>
                             <h1 className="text-lg font-black text-slate-900 dark:text-white tracking-tight">{navItems.find(n => n.id === activeTab)?.label}</h1>
                        </div>
                    )}
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {activeTab === 'dashboard' && (
                            <motion.div key="dash" variants={pageVariants} initial="hidden" animate="visible" exit="exit" className="space-y-3 md:space-y-5">

                                {/* Beautiful Welcome Banner */}
                                <div className="relative overflow-hidden rounded-[1.5rem] md:rounded-[2rem] bg-gradient-to-br from-[#f0f4ff] to-[#e0e7ff] dark:from-[#1e293b] dark:to-[#0f172a] p-5 md:p-8 flex items-center justify-between border border-blue-200/40 dark:border-white/5 shadow-sm">
                                    <div className="relative z-10">
                                        <h2 className="text-lg md:text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2 mb-1">
                                    ...
                                    <div className="relative z-10 w-16 h-16 md:w-28 md:h-28 hidden sm:flex items-center justify-center right-2 md:right-4">
                                        ...
                                        <span className="text-5xl md:text-7xl drop-shadow-2xl">📚</span>
                                    </div>
                                </div>

                                {/* Scaled down clean UI Grid */}
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-5 mt-2">
                                    {navItems.filter(item => item.id !== 'dashboard' && item.id !== 'settings').map((item, i) => {
                                        ...
                                        return (
                                            <motion.button
                                                ...
                                                className="bg-white dark:bg-[#0b1120] border border-slate-100 dark:border-white/5 p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] flex flex-col items-start justify-between text-left shadow-[0_4px_20px_rgba(0,0,0,0.03)] dark:shadow-none hover:shadow-lg transition-all aspect-square sm:aspect-auto sm:h-48 relative group"
                                            >
                                                ...
                                                <div className="absolute bottom-4 right-4 md:bottom-5 md:right-5 w-6 h-6 md:w-8 md:h-8 rounded-full bg-slate-50 dark:bg-[#151f32] flex items-center justify-center text-slate-400 group-hover:text-current transition-colors">
                                ...
                                {navItems.filter(item => item.id === 'settings').map(item => {
                                    ...
                                    return (
                                        <motion.button
                                            ...
                                            className="w-full bg-white dark:bg-[#0b1120] border border-slate-100 dark:border-white/5 p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] flex items-center gap-4 md:gap-5 text-left shadow-[0_4px_20px_rgba(0,0,0,0.03)] dark:shadow-none hover:shadow-lg transition-all relative group"
                                        >
                                            <div className={`w-10 h-10 md:w-14 md:h-14 rounded-full flex items-center justify-center shrink-0 ${item.bg}`}>
                                                <Icon className={item.color} size={20} strokeWidth={2.5} />
                                            </div>
                                            <div className="flex-1 pr-10">
                                                <span className="font-bold text-sm md:text-lg text-slate-900 dark:text-white block mb-0.5">{item.label}</span>
                                                <span className="text-xs md:text-sm text-slate-500 dark:text-slate-400 leading-snug line-clamp-1">{item.desc}</span>
                                            </div>
                                            <div className="absolute right-4 md:right-5 w-6 h-6 md:w-8 md:h-8 rounded-full bg-slate-50 dark:bg-[#151f32] flex items-center justify-center text-slate-400 group-hover:text-current transition-colors">
                                                <ChevronRight size={14} className={item.color} />
                                            </div>
                                        </motion.button>
                                    );
                        ...
                        {activeTab === 'assignments' && (
                            <motion.div key="assignments" variants={pageVariants} initial="hidden" animate="visible" exit="exit" className="space-y-3">
                                {assignments.length > 0 ? assignments.map((task, i) => (
                                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ ...springConfig, delay: Math.min(i * 0.02, 0.08) }} key={task.id}
                                        className="bg-white dark:bg-[#0b1120] border border-slate-100 dark:border-white/5 shadow-sm p-4 sm:p-6 rounded-xl sm:rounded-[1.5rem] flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 transition-colors">
                                        <div>
                                            <h3 className="text-base sm:text-xl font-bold text-slate-950 dark:text-white">{task.title}</h3>
                                            {task.description && <p className="text-xs sm:text-sm text-slate-600 dark:text-zinc-400 mt-1 sm:mt-2 leading-relaxed">{task.description}</p>}
                                        </div>
                                        <div className="shrink-0 sm:text-right flex flex-row sm:flex-col justify-between items-center sm:items-end mt-2 sm:mt-0">
                                            <span className="text-[10px] uppercase font-bold bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400 px-2.5 py-1 rounded-lg border border-blue-100 dark:border-blue-500/20">
                                                Homework
                                            </span>
                                            <div className="text-[11px] sm:text-xs font-bold text-slate-500 dark:text-zinc-500 sm:mt-2">
                                                {new Date(task.createdAt).toLocaleDateString()}
                                            </div>
                                        </div>
                                    </motion.div>
                                )) : (
                                    <div className="text-center py-20 sm:py-32 border-2 border-dashed border-slate-300/70 dark:border-white/10 rounded-2xl sm:rounded-[2.5rem] bg-white/40 dark:bg-white/[0.02] backdrop-blur-sm">
                                        <BookOpen size={48} className="mx-auto mb-4 text-slate-300 dark:text-white/20" />
                                        <p className="text-slate-500 dark:text-zinc-400 font-bold text-sm sm:text-lg">No homework assigned yet!</p>
                                    </div>
                                )}
                            </motion.div>
                        )}
                                ...
                                <div className="bg-white dark:bg-[#0b1120] border border-slate-100 dark:border-white/5 shadow-sm p-5 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem]">
                                    <div className="flex justify-between items-center mb-5 md:mb-10">
                                        ...
                                        <div className="flex gap-2 md:gap-3">
                                            <motion.button whileTap={{ scale: 0.9 }} onClick={() => setViewDate(new Date(viewDate.setMonth(viewDate.getMonth() - 1)))} className="p-2.5 md:p-3 bg-slate-50 text-slate-600 border border-slate-200 dark:bg-white/5 dark:border-white/10 dark:text-white rounded-full hover:bg-slate-100 dark:hover:bg-white/10 transition"><ChevronLeft size={18} /></motion.button>
                                            <motion.button whileTap={{ scale: 0.9 }} onClick={() => setViewDate(new Date(viewDate.setMonth(viewDate.getMonth() + 1)))} className="p-2.5 md:p-3 bg-slate-50 text-slate-600 border border-slate-200 dark:bg-white/5 dark:border-white/10 dark:text-white rounded-full hover:bg-slate-100 dark:hover:bg-white/10 transition"><ChevronRight size={18} /></motion.button>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-7 gap-y-5 md:gap-y-8 text-center">
                                        {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => <span key={i} className="text-[10px] md:text-sm font-black text-emerald-500 uppercase">{d}</span>)}
                                        ...
                                        {Array.from({ length: new Date(viewDate.getFullYear(), viewDate.getMonth() + 1, 0).getDate() }).map((_, i) => {
                                            ...
                                            return (
                                                <div key={day} className="flex flex-col items-center gap-1.5 md:gap-2">
                                                    <span className={`text-xs md:text-base font-bold ${status ? 'text-slate-900 dark:text-white' : 'text-slate-400 dark:text-zinc-500'}`}>{day}</span>
                                                    {status === 'present' && <motion.div initial={{ scale: 0 }} animate={{ scale: 1, transition: fastSpringConfig }} className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.8)]" />}
                                                    {status === 'absent' && <motion.div initial={{ scale: 0 }} animate={{ scale: 1, transition: fastSpringConfig }} className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.8)]" />}
                                                </div>
                                            );
                                        })}
                                    </div>
                        ...
                        {activeTab === 'fees' && (
                            <motion.div key="fees" variants={pageVariants} initial="hidden" animate="visible" exit="exit">
                                <div className="bg-white dark:bg-[#0b1120] border border-slate-100 dark:border-white/5 shadow-sm rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden">
                                    <div className="p-5 md:p-8 border-b border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02]">
                                        <h2 className="text-lg md:text-2xl font-black text-slate-900 dark:text-white tracking-tight">Academic Fee {new Date().getFullYear()}</h2>
                                    </div>
                                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-5 p-4 md:p-8">
                                        {["jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec"].map((m, i) => {
                                            ...
                                            return (
                                                <motion.div
                                                    ...
                                                    key={m} className={`p-3 md:p-5 rounded-xl md:rounded-2xl border flex flex-col xl:flex-row justify-between items-start xl:items-center gap-2 xl:gap-0 ${status === 'paid' ? 'bg-slate-50 border-slate-200 dark:bg-white/5 dark:border-white/10' : 'bg-white border-slate-100 dark:bg-[#06080f] dark:border-white/5'}`}
                                                >
                                                    <div className="flex items-center gap-2 md:gap-4">
                                                        <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-[10px] md:text-xs font-black uppercase ${status === 'paid' ? 'bg-slate-200 text-slate-800 dark:bg-zinc-700 dark:text-white' : 'bg-slate-100 text-slate-500 dark:bg-white/5 dark:text-zinc-400'}`}>{m.substring(0, 3)}</div>
                                                        <span className="capitalize font-bold text-sm md:text-base text-slate-900 dark:text-zinc-100">{m}</span>
                                                    </div>
                                                    {status === 'paid' ? <CheckCircle size={18} className="text-emerald-500 self-end xl:self-auto" /> : <div className="text-[10px] md:text-[11px] bg-slate-100 text-slate-500 dark:bg-white/10 dark:text-zinc-400 px-2.5 py-1 md:px-3 md:py-1.5 rounded-full font-bold uppercase border border-slate-200 dark:border-white/5 self-end xl:self-auto">Due</div>}
                                                </motion.div>
                                            )
                                        })}
                                    </div>
                        ...
                        {activeTab === 'analytics' && (
                            <motion.div key="analytics" variants={pageVariants} initial="hidden" animate="visible" exit="exit" className="space-y-4 md:space-y-6">

                                <div className="bg-gradient-to-br from-slate-900 to-black dark:from-zinc-800/80 dark:to-black/80 text-white p-6 md:p-10 rounded-[1.5rem] md:rounded-[2.5rem] shadow-xl relative overflow-hidden border border-slate-800 dark:border-white/10">
                                    <div className="absolute top-[-20%] right-[-10%] w-48 h-48 md:w-64 md:h-64 bg-white/10 rounded-full blur-[60px] mix-blend-overlay"></div>
                                    <div className="relative z-10">
                                        <div className="flex items-center gap-2 mb-2 md:mb-3 opacity-90">
                                            <TrendingUp size={16} className="md:w-5 md:h-5" />
                                            <span className="text-xs md:text-sm font-black uppercase tracking-widest">Overall Performance</span>
                                        </div>
                                        <div className="text-5xl md:text-8xl font-black tracking-tighter drop-shadow-lg">
                                            {currentStudent.performance || 0}<span className="text-2xl md:text-3xl text-white/60">%</span>
                                        </div>
                                        <p className="text-xs md:text-base font-medium text-slate-400 dark:text-zinc-400 mt-2 md:mt-4 max-w-sm">Average based on all recorded test scores throughout the session.</p>
                                    </div>
                                </div>

                                <div className="bg-white dark:bg-[#0b1120] border border-slate-100 dark:border-white/5 shadow-sm p-5 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem]">
                                    <div className="mb-6 md:mb-8">
                                        <p className="text-[10px] md:text-xs font-black text-rose-500 uppercase tracking-widest">Test History</p>
                                        <h3 className="text-lg md:text-2xl font-black text-slate-900 dark:text-white mt-1">Performance Over Time</h3>
                                    </div>
                                        ...
                                        {currentStudent.performanceHistory && currentStudent.performanceHistory.length > 0 ? (
                                            <ResponsiveContainer width="100%" height="100%">
                                                <LineChart data={currentStudent.performanceHistory}>
                                                    ...
                                                    <Tooltip
                                                        contentStyle={{ backgroundColor: 'rgba(15, 23, 42, 0.9)', backdropFilter: 'blur(16px)', borderColor: 'rgba(255,255,255,0.1)', borderRadius: '12px', color: '#fff', boxShadow: '0 8px 32px rgba(0,0,0,0.2)', fontSize: '12px' }}
                                                        itemStyle={{ color: '#f43f5e', fontWeight: '900' }}
                                                    />
                                                    <Line type="monotone" dataKey="score" stroke="#f43f5e" strokeWidth={4} dot={{ r: 4, fill: '#fff', stroke: '#f43f5e', strokeWidth: 2 }} activeDot={{ r: 7, fill: '#f43f5e' }} />
                                                </LineChart>
                                            </ResponsiveContainer>
                                        ) : (
                                            <div className="h-full flex items-center justify-center text-slate-400 dark:text-zinc-500 text-xs md:text-sm font-bold border-2 border-dashed border-slate-200 dark:border-white/10 rounded-2xl md:rounded-[2rem] bg-slate-50 dark:bg-white/[0.02]">
                                ...
                                <div className="grid grid-cols-2 gap-3 md:gap-6">
                                    <div className="bg-white dark:bg-[#0b1120] border border-slate-100 dark:border-white/5 shadow-sm p-5 md:p-8 rounded-[1.5rem] md:rounded-[2rem] flex flex-col justify-between h-32 md:h-48">
                                        <div className="w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-[1rem] bg-emerald-50 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400 flex items-center justify-center border border-emerald-100 dark:border-emerald-500/20"><CheckCircle size={18} className="md:w-6 md:h-6" /></div>
                                        <div>
                                            <div className="text-3xl md:text-5xl font-black tracking-tight text-slate-900 dark:text-white">{stats.attPercent}<span className="text-base md:text-2xl text-slate-400 dark:text-zinc-500 ml-1">%</span></div>
                                            <div className="text-[10px] md:text-xs font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest mt-1 md:mt-2">Attendance</div>
                                        </div>
                                    </div>
                                    <div className="bg-white dark:bg-[#0b1120] border border-slate-100 dark:border-white/5 shadow-sm p-5 md:p-8 rounded-[1.5rem] md:rounded-[2rem] flex flex-col justify-between h-32 md:h-48">
                                        <div className="w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-[1rem] bg-slate-100 text-slate-600 dark:bg-zinc-500/20 dark:text-zinc-300 flex items-center justify-center border border-slate-200 dark:border-zinc-500/20"><IndianRupee size={18} className="md:w-6 md:h-6" /></div>
                                        <div>
                                            <div className="text-3xl md:text-5xl font-black tracking-tight text-slate-900 dark:text-white">{stats.feePaid}</div>
                                            <div className="text-[10px] md:text-xs font-black text-slate-500 dark:text-zinc-400 uppercase tracking-widest mt-1 md:mt-2">Months Paid</div>
                                        </div>
                                    </div>
                                </div>
                            </motion.div>
                        ...
                        {activeTab === 'timing' && (
                            <motion.div key="timing" variants={pageVariants} initial="hidden" animate="visible" exit="exit" className="flex flex-col items-center justify-center min-h-[50vh] py-10">
                                <div className="relative">
                                    <div className="absolute inset-0 bg-yellow-500/20 blur-2xl md:blur-3xl rounded-full"></div>
                                    <div className="w-20 h-20 md:w-28 md:h-28 bg-white dark:bg-[#0b1120] border border-slate-100 dark:border-white/10 text-yellow-500 rounded-full flex items-center justify-center mb-6 md:mb-8 shadow-lg relative z-10"><Clock size={36} className="md:w-12 md:h-12" /></div>
                                </div>
                                <h2 className="text-2xl md:text-3xl font-black mb-1 md:mb-2 text-slate-900 dark:text-white">Class Schedule</h2>
                                <p className="text-slate-500 dark:text-zinc-400 font-medium mb-8 md:mb-10 text-sm md:text-lg">{currentStudent.batchName} Batch</p>
                                <div className="flex items-center gap-2 md:gap-4">
                                    <div className="bg-white dark:bg-[#0b1120] border border-slate-100 dark:border-white/5 p-4 md:p-6 rounded-2xl md:rounded-3xl text-center w-32 md:w-40 shadow-sm">
                                        <div className="text-[10px] md:text-xs text-yellow-500 uppercase font-black mb-2 md:mb-3 tracking-widest">Start Time</div>
                                        <div className="text-lg md:text-2xl font-black text-slate-900 dark:text-white">{currentStudent.batchTiming?.start ? new Date(`1970-01-01T${currentStudent.batchTiming.start}`).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : "--:--"}</div>
                                    </div>
                                    <div className="h-1 w-4 md:h-1.5 md:w-6 bg-slate-200 dark:bg-white/20 rounded-full"></div>
                                    <div className="bg-white dark:bg-[#0b1120] border border-slate-100 dark:border-white/5 p-4 md:p-6 rounded-2xl md:rounded-3xl text-center w-32 md:w-40 shadow-sm">
                                        <div className="text-[10px] md:text-xs text-yellow-500 uppercase font-black mb-2 md:mb-3 tracking-widest">End Time</div>
