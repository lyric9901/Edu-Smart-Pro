...
function StudentContent() {
    ...
    const navItems = [
        ...
        { id: 'assignments', icon: <BookOpen size={28} />, label: 'Homework', color: 'text-zinc-300', bg: 'bg-zinc-500/10' },
        { id: 'attendance', icon: <CheckCircle size={28} />, label: 'Attendance', color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
        { id: 'fees', icon: <IndianRupee size={28} />, label: 'Fees', color: 'text-zinc-300', bg: 'bg-zinc-500/10' },
        { id: 'notices', icon: <Bell size={28} />, label: 'Inbox', color: 'text-blue-400', bg: 'bg-blue-500/10' },
        { id: 'analytics', icon: <PieChart size={28} />, label: 'Stats', color: 'text-zinc-300', bg: 'bg-zinc-500/10' },
        { id: 'timing', icon: <Clock size={28} />, label: 'Time', color: 'text-zinc-400', bg: 'bg-zinc-500/10' },
    ];

    return (
        <div className="flex min-h-screen font-sans transition-colors duration-300 select-none bg-black text-zinc-100 relative overflow-hidden">
            
            {/* Ambient Glowing Orbs for Glassmorphism Background - Swapped Purple for sleek Monochrome/Blue */}
            <div className="fixed inset-0 z-0 pointer-events-none">
                ...
                <div className="absolute bottom-[-10%] right-[-10%] w-[40rem] h-[40rem] bg-blue-900/10 rounded-full blur-[140px] mix-blend-screen"></div>
                                ...
                                {assignments.length > 0 ? assignments.map((task, i) => (
                                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} key={task.id}
                                        ...
                                        <div className="shrink-0 sm:text-right flex flex-row sm:flex-col justify-between items-center sm:items-end">
                                            <span className="text-[11px] uppercase font-bold bg-white/10 text-zinc-300 px-3 py-1.5 rounded-lg border border-white/10 shadow-sm">
                                                Homework
                                            </span>
                            ...
                            <motion.div key="att" variants={pageVariants} initial="hidden" animate="visible" exit="exit">
                                <div className="bg-white/[0.04] backdrop-blur-2xl p-6 md:p-8 rounded-[2rem] border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
                                    ...
                                    <div className="grid grid-cols-7 gap-y-8 text-center">
                                        {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => <span key={i} className="text-xs md:text-sm font-black text-blue-400 uppercase">{d}</span>)}
                                        ...
                                        {Array.from({ length: new Date(viewDate.getFullYear(), viewDate.getMonth() + 1, 0).getDate() }).map((_, i) => {
                                            ...
                                            return (
                                                <div key={day} className="flex flex-col items-center gap-2">
                                                    ...
                                                    {status === 'absent' && <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-2 h-2 md:w-2.5 md:h-2.5 rounded-full bg-zinc-500 shadow-[0_0_12px_rgba(113,113,122,0.8)]" />}
                                                </div>
                                            );
                                        })}
                                    </div>
                        ...
                        {activeTab === 'analytics' && (
                            <motion.div key="analytics" variants={pageVariants} initial="hidden" animate="visible" exit="exit" className="space-y-6">
                                ...
                                <div className="bg-white/[0.04] backdrop-blur-2xl p-6 md:p-8 rounded-[2.5rem] border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
                                    <div className="mb-8">
                                        <p className="text-xs font-black text-blue-400 uppercase tracking-widest">Test History</p>
                                        ...
                                        {currentStudent.performanceHistory && currentStudent.performanceHistory.length > 0 ? (
                                            <ResponsiveContainer width="100%" height="100%">
                                                <LineChart data={currentStudent.performanceHistory}>
                                                    ...
                                                    />
                                                    <Line type="monotone" dataKey="score" stroke="#60a5fa" strokeWidth={5} dot={{ r: 6, fill: '#000', stroke: '#60a5fa', strokeWidth: 3 }} activeDot={{ r: 9, fill: '#fff' }} />
                                                </LineChart>
                                            </ResponsiveContainer>
                                        ) : (
                        ...
                        {activeTab === 'timing' && (
                            <motion.div key="timing" variants={pageVariants} initial="hidden" animate="visible" exit="exit" className="flex flex-col items-center justify-center min-h-[50vh]">
                                <div className="relative">
                                    ...
                                    <div className="w-28 h-28 bg-white/5 backdrop-blur-2xl border border-white/20 text-zinc-300 rounded-full flex items-center justify-center mb-8 shadow-[0_8px_32px_rgba(0,0,0,0.3)] relative z-10"><Clock size={48} /></div>
                                </div>
                                ...
                                <div className="flex items-center gap-4">
                                    <div className="bg-white/[0.04] backdrop-blur-xl p-6 rounded-3xl border border-white/10 text-center w-40 shadow-xl">
                                        <div className="text-xs text-blue-400 uppercase font-black mb-3 tracking-widest">Start Time</div>
                                    ...
                                    <div className="bg-white/[0.04] backdrop-blur-xl p-6 rounded-3xl border border-white/10 text-center w-40 shadow-xl">
                                        <div className="text-xs text-blue-400 uppercase font-black mb-3 tracking-widest">End Time</div>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/60 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4">
                        <motion.div initial={{ y: "100%", scale: 0.95 }} animate={{ y: 0, scale: 1 }} exit={{ y: "100%", scale: 0.95 }} className="bg-[#0f0f13]/80 backdrop-blur-3xl w-full sm:w-[28rem] rounded-t-[2.5rem] sm:rounded-[2.5rem] p-6 md:p-8 shadow-[0_16px_60px_rgba(0,0,0,0.6)] border border-white/10">
                            ...
                            <div className="space-y-4">
                                {students.map((s, idx) => (
                                    <button key={idx} onClick={() => switchStudent(idx)} className={`w-full flex items-center gap-4 p-4 rounded-2xl border backdrop-blur-md transition-all ${idx === activeStudentIndex ? 'border-white/30 bg-white/10 text-white shadow-[0_4px_20px_rgba(255,255,255,0.1)]' : 'border-white/10 bg-white/5 hover:bg-white/10 text-zinc-300'}`}>
                        ...
                        <motion.div initial={{ y: "100%", scale: 0.95 }} animate={{ y: 0, scale: 1 }} exit={{ y: "100%", scale: 0.95 }} className="bg-[#0f0f13]/80 backdrop-blur-3xl w-full sm:w-[28rem] rounded-t-[2.5rem] sm:rounded-[2.5rem] p-6 md:p-8 shadow-[0_16px_60px_rgba(0,0,0,0.6)] border border-white/10">
                            ...
                            {settingsView === "password" && (
                                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                                    ...
                                    <div className="space-y-4">
                                        <input type="password" placeholder="Current Password" value={passForm.current} onChange={e => setPassForm({ ...passForm, current: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-4 text-sm outline-none focus:border-blue-500 text-white placeholder:text-zinc-500 backdrop-blur-sm" />
                                        <input type="password" placeholder="New Password" value={passForm.new} onChange={e => setPassForm({ ...passForm, new: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-4 text-sm outline-none focus:border-blue-500 text-white placeholder:text-zinc-500 backdrop-blur-sm" />
                                        <input type="password" placeholder="Confirm New Password" value={passForm.confirm} onChange={e => setPassForm({ ...passForm, confirm: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-4 text-sm outline-none focus:border-blue-500 text-white placeholder:text-zinc-500 backdrop-blur-sm" />
                                        ...
                                        <button onClick={handlePasswordChange} className="w-full bg-white/10 text-white py-4 rounded-xl text-base font-bold hover:bg-white/20 transition border border-white/20 mt-4">Save Password</button>
                                    </div>
                                </motion.div>
                            )}
                        </motion.div>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/60 backdrop-blur-md flex items-center justify-center p-6">
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="bg-[#0f0f13]/80 border border-white/10 backdrop-blur-3xl w-full max-w-sm rounded-[2.5rem] p-8 shadow-[0_16px_60px_rgba(0,0,0,0.6)]">
                            ...
                            <form onSubmit={addNewChild} className="space-y-4">
                                <input required placeholder="Student Name" className="w-full p-4 bg-white/5 border border-white/10 rounded-xl text-white outline-none focus:border-blue-500 placeholder:text-zinc-500 backdrop-blur-sm" onChange={e => setAddForm({ ...addForm, name: e.target.value })} />
                                <input required placeholder="Phone Number" className="w-full p-4 bg-white/5 border border-white/10 rounded-xl text-white outline-none focus:border-blue-500 placeholder:text-zinc-500 backdrop-blur-sm" onChange={e => setAddForm({ ...addForm, phone: e.target.value })} />
                                ...
                                <div className="flex gap-3 pt-4">
                                    ...
                                    <button className="flex-1 bg-white/10 border border-white/20 text-white py-4 rounded-2xl font-bold hover:bg-white/20 transition-all">Add</button>
                                </div>
                            </form>
                        </motion.div>
                    </motion.div>
