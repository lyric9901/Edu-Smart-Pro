...
import { doc, getDoc, collection, getDocs, onSnapshot, updateDoc } from "firebase/firestore"; // <-- Firestore imports
...
import {
    ...
    Maximize, Minimize, Home, BookOpen
} from "lucide-react";


// --- ZOD SCHEMAS ---
// Rule: Name must be at least 3 letters. Phone must be exactly 10 digits.
const childSchema = z.object({
    name: z.string().min(3, "Name must be at least 3 characters long"),
    phone: z.string().regex(/^\d{10}$/, "Phone number must be exactly 10 digits"),
});

// Rule: Passwords must be at least 6 characters.
...
function StudentContent() {
    ...
    // --- OPTIMIZATION: REAL-TIME FIRESTORE SYNC ---
    useEffect(() => {
        if (!currentStudent?.institutionCode || !currentStudent?.batchId || !currentStudent?.id) return;

        // 1. Listen to Batch Document (Gets Student Data + Assignments + Timings)
        ...
        const unsubBatch = onSnapshot(batchRef, (docSnap) => {
            if (docSnap.exists()) {
                ...
                // Sync specific student from the array
    ...
    // NEW ADD CHILD FOR FIRESTORE
    const addNewChild = async (e) => {
        e.preventDefault();
        if (!currentStudent?.institutionCode) return;

        try {
            // 1. ZOD VALIDATION: This will throw an error if rules are broken
            const validatedData = childSchema.parse({
                name: addForm.name,
                phone: addForm.phone
            });

            // 2. If it passes, run your normal Firebase logic using the validated data
            ...
            if (found) {
                if (students.some(s => s.id === found.id)) {
                    toast.error("Student already added to your account."); // <-- Using Toast now!
                    return;
                }
                ...
                toast.success("Child account linked successfully!"); // <-- Using Toast now!
            } else {
                toast.error("Student not found in the institute database.");
            }

        } catch (err) {
            // 3. HANDLE ZOD ERRORS cleanly
            if (err instanceof z.ZodError) {
                toast.error(err.errors[0].message); // Shows exactly what rule they broke
            } else {
    ...
    // NEW PASSWORD LOGIC FOR FIRESTORE (Keeps password entirely internal)
    const handlePasswordChange = async () => {
        try {
            // 1. Standard JS Validation (No Zod)
            ...
            // 2. Firebase Update (With Branch Support)
            if (currentStudent?.institutionCode && currentStudent?.batchId && currentStudent?.id) {

                // Check if the student belongs to a specific branch, otherwise use the old path
